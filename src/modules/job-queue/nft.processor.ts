  ...
  constructor(
    ...
    this.queueService
      ...
      .process(
        'nft',
        async (
          job: Job<{ txCreation: string; type: 'ERC721' | 'ERC1155' }>,
          done,
        ) => {
          try {
            ...
            if (
              collectionData &&
              (('erc721Tokens' in collectionData &&
                collectionData.erc721Tokens.length > 0) ||
                ('erc1155Tokens' in collectionData &&
                  collectionData.erc1155Tokens.length > 0))
            ) {
